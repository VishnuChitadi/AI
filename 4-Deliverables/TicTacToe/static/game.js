...
async function startGame(human) {
  ...
  // If AI goes first (human chose O)
...
async function handleCellClick(macroIdx, row, col) {
  ...
  const macroRow = Math.floor(macroIdx / 3);
  const macroCol = macroIdx % 3;
...
async function doAiMove() {
  ...
  const iterations = parseInt(iterInput.value, 10) || 5000;
...
function render() {
  ...
  for (let m = 0; m < 9; m++) {
    ...
    // Determine if this sub-board is the active one
    ...
    // Macro overlay for won/drawn boards
    if (macroStatus !== null) {
      const overlay = document.createElement('div');
      overlay.className = `macro-overlay ${macroStatus}`;
      overlay.textContent = macroStatus === 'draw' ? '—' :
                            macroStatus === 'X'    ? '✕' : '○';
      subDiv.appendChild(overlay);
    }
    ...
    for (let r = 0; r < 3; r++) {
      for (let c = 0; c < 3; c++) {
        ...
        } else if (isActive && macroStatus === null) {
...
function setBoardInteractive(on) {
  metaBoard.style.pointerEvents = on ? '' : 'none';
}
