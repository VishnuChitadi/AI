...
for e in range(n_epochs):
    ...
    if (e + 1) % 100 == 0:
        acc = evaluate(hidden_weights, output_weights, X_train, y_train)
        print(f"Epoch {e+1:5d} | Avg MSE: {avg_err:.6f} | Train Acc: {acc:.2%}")

# --- Final test evaluation ---
test_acc = evaluate(hidden_weights, output_weights, X_test, y_test)
...
plt.title("Training Error over Epochs — Iris Classification")
plt.tight_layout()
plt.savefig("iris_training_error.png", dpi=150)
plt.show()
print("Plot saved to iris_training_error.png")
